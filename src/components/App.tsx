...
import WPThemeDownload from "./wp-theme-download/wp-theme-download";
...
function App() {
  ...
  // WordPress Theme Conversion State
  const [wpThemeReady, setWpThemeReady] = useState(false);
  const [wpThemeData, setWpThemeData] = useState<{
    filename: string;
    downloadUrl: string;
    createdAt: Date;
    themeInfo: {
  ...
  // WordPress Theme Conversion Handlers
  const handleWordPressConversion = async () => {
    if (!html || html === defaultHTML) {
      toast.warn("Please generate some content first");
      return;
    }
    ...
    try {
      // Helper function to try both proxy and direct backend URL
      const tryFetch = async (endpoint: string, options: RequestInit) => {
        try {
      ...
      try {
        analysisResponse = await tryFetch('/api/analyze-html', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ html })
        });
        if (!analysisResponse.ok) {
          throw new Error(`HTML analysis failed with status: ${analysisResponse.status}`);
        }
      } catch (err) {
        handleError(err, "HTML analysis failed. Please check the console for details.");
      ...
      if (!analysis.canConvert) {
        // This is not an exception/error, but a business logic failure, toast.error is fine.
        toast.error("HTML structure not suitable for WordPress conversion");
        setIsConverting(false);
        return;
      }
      ...
      try {
        conversionResponse = await tryFetch('/api/convert-to-wordpress', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            html,
            analysis,
            themeOptions: {
              name: 'R3Code Generated Theme',
              description: 'WordPress theme generated by R3Code AI',
              version: '1.0.0'
            }
          })
        });
        if (!conversionResponse.ok) {
          throw new Error(`WordPress conversion failed with status: ${conversionResponse.status}`);
        }
      } catch (err) {
        handleError(err, "WordPress conversion failed. Please check the console for details.");
        setIsConverting(false);
        return;
      }

      // Get the blob URL for download
      try {
        const blob = await conversionResponse.blob();
        const downloadUrl = URL.createObjectURL(blob);

        // Set theme data for download banner
        setWpThemeData({
          filename: 'r3code-wp-theme.zip',
          downloadUrl,
          createdAt: new Date(),
          themeInfo: {
            name: 'R3Code Generated Theme',
        ...
        setWpThemeReady(true);
        toast.success("WordPress theme generated successfully!");
      } catch (err) {
        handleError(err, "Failed to process WordPress theme data.");
        setIsConverting(false);
        return;
      }

    } catch (err) { // This catch is for any unexpected errors during the process
      handleError(err, "An unexpected error occurred during WordPress conversion.");
    } finally {
      // Ensure isConverting is always reset.
      setIsConverting(false);
    }
  };

  // Download handler
  const handleWPThemeDownload = () => {
    if (wpThemeData) {
      const link = document.createElement('a');
      link.href = wpThemeData.downloadUrl;
      link.download = wpThemeData.filename;
      ...
      // Optional: Auto-dismiss after download
      setTimeout(() => {
        setWpThemeReady(false);
        if (wpThemeData.downloadUrl) {
          URL.revokeObjectURL(wpThemeData.downloadUrl);
        }
        setWpThemeData(null);
      }, 2000);
    }
  ...
  return (
    <div className="vh-100 d-flex flex-column" style={{ background: 'var(--ids-gray-900)' }}>
      <Header
        ...
        onWPConvert={handleWordPressConversion}
      ...
      {/* WordPress Theme Download Banner */}
      {wpThemeReady && wpThemeData && (
        <WPThemeDownload
          themeData={wpThemeData}
          onDownload={handleWPThemeDownload}
          onDismiss={() => setWpThemeReady(false)}
        />
      )}
