...
function App() {
  ...
  const handleWordPressConversion = async () => {
    ...
    try {
      ...
      // Step 1: Analyze HTML
      const analysisResponse = await tryFetch('/api/analyze-html', {
        method: 'POST',
      ...
      const conversionResponse = await tryFetch('/api/convert-to-wordpress', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          html,
      ...
      toast.error("Failed to convert to WordPress theme");
    } finally {
